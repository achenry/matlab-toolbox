function Af_EditADriver(lines, edits, newname, defname, templateDir)

%% Open Files
fidDefault  = fopen(fullfile(templateDir,[defname,'.dvr']),'r');
fidNew      = fopen([newname,'.dvr'],'w+');
fidDesc     = fopen(fullfile(templateDir,'ADriver_desc.dat'));
    ...
    while 1
        ...
        for iLines = 1:length(lines)
            if ~isempty(strfind(tline,lines{iLines}))
                editedLine = true;
                %find description
                fidDesc     = fopen(fullfile(templateDir,'ADriver_desc.dvr'));
                while 1
