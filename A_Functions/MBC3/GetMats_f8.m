...
for iFile = 1:NAzimStep
    ...
    if (isfield(data(iFile), 'u_op'))
        uop(:,iFile) = cell2mat(data(iFile).u_op);
    end
    
    if (isfield(data(iFile), 'y_op'))
        yop(:,iFile) = cell2mat(data(iFile).y_op);
    end
...
%     [RotTripletIndicesOutput, NRotTripletOutput] = findBladeTriplets(data(1).y_rotFrame,DescOutput );
      RotTripletIndicesOutput = [find(strcmp(DescOutput,'ED RootMyb1, (kN·m)')),find(strcmp(DescOutput,'ED RootMyb2, (kN·m)')),find(strcmp(DescOutput,'ED RootMyb3, (kN·m)'))];
      NRotTripletOutput = 1;
end
