function M_GetMats(A_CD,filename, SaveDir, SaveName, n)
% M_GetMats.m
% Edited by J. Aho, CU


% Call new M_GetMats
%     Save new .mat file for for each call
%     Analyze Linearization files
...
FASTLinName = [ filename,'.lin'];   % FAST (.lin) filename
...
%% Run other post processes

% % Call Eianalysis
% [EigenVects,eigenVects,Evals,eigenVals,DampedFrequencies,DampedFreqs_Hz,NaturalFrequencies,...
% NaturalFreqs_Hz,DecrementRate,DampRatios,MagnitudeModes,PhaseModes_deg,nss] = M_eianalysis(AvgAMat)

% Gets eigens for averaged matrix, the one above doesnt do this?
M_cce
...
% % 
% [Aout] = row_col_xtion(Ainp, new_seq, rc_id, rev_seq)
% % re-sequence rows or columns of the input matrix

% % Wont be using right away
% [new_seq] = get_new_seq(rot_q_triplet,ntot)
% % form new_sequence vector with non-rot identifiers followed by rot identifiers


mkdir([A_CD,'/Linear/'],SaveDir)

save([A_CD,'/Linear/',SaveDir,'/',SaveName,num2str(n)]) % , 'AMat', 'AvgAMat', 'AvgBMat', 'AvgBdMat',...
    %'AvgCMat', 'AvgDMat', 'AvgDdMat', 'RotSpeed' )

% ----------- We're finished ----------------------------------------------

% Tell the user that we are finished:
disp( '                                      ' );
disp( ['M_GetMats.m completed for file ',num2str(n)] );
