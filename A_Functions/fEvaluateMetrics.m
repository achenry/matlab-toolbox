...
fileIndSpec = NaN(nRefFreq,nVars)             ;
fileIndULS  = NaN(1,nVars)                    ;
Freq        = R.Freq                          ;
...
for iVar = 1:nVars
    ...
    elseif strfind(Kind,'FRQ')==1
        ...
        for iFreq=1:nRefFreq
            ...
            [SpecAMax(iFreq,iVar),fileIndSpec(iFreq,iVar)] = max(squeeze(SpecA(iFreq,:,iVar)));
...
M.IMain      = IMain     ;
M.SpecA      = SpecA      ;
M.FileIndSpec= fileIndSpec;
M.FileIndULS = fileIndULS;
M.SpecAMax   = SpecAMax   ;
